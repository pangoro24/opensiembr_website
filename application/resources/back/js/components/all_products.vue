                ...
                <v-skeleton-loader type="table" :loading="this.$root.skeletonLoading" transition="scale-transition">
                    <v-card outlined elevation="4">
                        <v-card-title class="card-header">
                            <span class="body-1">Todos los productos creadoss</span>
                        </v-card-title>
                            <v-data-table
                                :headers="headers"
                                :items="desserts"
								...
								<template v-slot:item.description="{ item }">
									<span v-html="item.description.substring(0,190)"></span>
								</template>
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            axios.get(this.$root.base_url + 'shop/get')
            .then(res => {
        ...
        methods: {
            edit: function (item)
            {
                location.href = '/admin/shop/edit/' + item;
            }
        },
    }
</script>
